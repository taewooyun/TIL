...
## [개념 슬롯]
### 임시 이름:
eidianness (byte order)
### 한 줄 요약:
하나의 숫자를 끊어서 저장할 때 큰수부터 저장 하면 big endian 작은 수 부터 저장하면 little endian
...
NETWORK : big endian
ARM : little endian(default) / big endian
...
### 내용:
백그라운드에서 조용히 돌아가는 프로세스 생성
시스템디로 서비스를 활용하여 부팅시 자동으로 실행되는 프로세스를 만들기
...
void pinMode(int p, int mode)
{
    ...
    else
    {
        ...
        GPIOA->CRL &= ~(0xF << 0);
        GPIOA->CRL |=  (0x5 << 0);   // MODE=01, CNF=01
    }
}
...
void digitalWrite(int pin, int set)
{
...
void read_dht_data(void)
{
        ...
        (data[4] == ((data[0] + data[1] + data[2] + data[3]) & 0xFF)))
    {
        ...
        if (h > 1000) h = data[0] * 10;

        unsigned c = (((data[2] & 0x7F) << 8) + data[3]);
        if (c > 1250) c = data[2] * 10;
        if (data[2] & 0x80) c = -c;
